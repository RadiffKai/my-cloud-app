...
from auth.auth import get_password_hash,authenticate_user,create_access_token,create_refresh_token,hash_refresh_token,decode_refresh_token,create_reset_token,verify_reset_token
...
def refresh(token:schemas.refreshRequest, db:Session = Depends(get_db)):
    ...
    if sessionstart + timedelta(days= 3) < datetime.now(timezone.utc):
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail="Login")
    ...
    if db_token.token_hash != hash_refresh_token(refresh_token):
        ...
        db.add(db_token)
        db.commit()
        raise HTTPException(status.HTTP_401_UNAUTHORIZED,detail="Token reuse,Please Login again")
    new_refresh_token,new_jti,new_expires_at,new_orig_iat = create_refresh_token(user_id=user_id, sub = user_id,orig_iat = orig_iat)
    db_token.revoked = True
...
def logout(token:schemas.Logout, db:Session = Depends(get_db)):
    ...
    if not token_str:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED,detail="login to Logout")
...
async def sendresetemail(request:schemas.resetRequest, background_task: BackgroundTasks,db:Session = Depends(get_db)):
    ...
    reset_link = f"http://localhost:8000/auth/passwordreset?token={token}"
